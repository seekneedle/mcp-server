...
async def generate_map(markers: List[str], lines: List[str] = []) -> str:
    ...
    参数:
        markers: 在地图上的标记点的名称和坐标，每个标记点格式为"该标记点涉及的所有天数和名称,经度,纬度"，同一个地点坐标合并到一个标记点，比如第1天出发地和第6天返回地都是北京，则合并为一个标记，并且将所有该点涉及的天数都合并到该点名称，如“第1天和第6天北京”，“第2-6天上海”，不要遗漏天数。
           示例: ["第1天和第6天北京,116.407,39.904", "第2-6天上海,121.473,31.230"]
...
async def query_weather_for_coordinates(coordinates: List[Dict[str, str]]) -> List[Dict]:
    """
    查询多个地点(带日期)的天气信息，如果一个地点涉及多个日期，请将列出每个地点日期，不要遗漏。
