...
KB_ID = config["video_id"]

async def video_search(query: str, video_num) -> List[Dict]:
    results = await retrieve_needle(query, index_id=KB_ID, top_k=video_num)
    all_results = []
    for result in results:
        ...
        link = f"{config['video_oss_link']}{file_name}"
        all_results.append({
            "title": title,
...
async def videos_search(queries: List[str], video_num: int) -> Dict[str, List[Dict]]:
    async def limited_search(query):
        async with semaphore:  # 获取信号量，如果已满则等待
            return await video_search(query, video_num)
